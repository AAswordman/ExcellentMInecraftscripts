 ...
 */
function encode(rawData: Uint8Array | Buffer | Array<any> | string): Array<any> {
    let dataType = typeof(rawData) == 'string' ? kString : kUint8Array
    , curIndex = 0
    , curBit = 0 // Points to current bit needed
    , curMask = 0b10000000
    , outData = []
    , getByte = dataType == kString ? (i: any) => (rawData as string).codePointAt(i) : (i: string | number) => (rawData as Uint8Array)[i as number]
    ;
 ...
 */
function encodeFromBase64(base64String: string): Array<any> {
 ...
 */
function utf8DataToString(data: Uint8Array | Buffer): string {
...
// For debugging.
function print7Bits(num: number | boolean) {
    return "0000000".substring(num.toString(2).length) + num.toString(2);
}

// For debugging.
function print8Bits(num: { toString: (arg0: number) => string; }) {
