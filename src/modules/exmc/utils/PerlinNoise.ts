...
export default class PerlinNoise {
    ...
    private s_curve(t: number) { return t * t * (3 - t - t); }
    ...
    public noise(x: number, y?: number, z?: number) {
        if (y !== undefined && z !== undefined) {
            ...
            q = this.G(b00 + bz); u = rx0 * q[0] + ry0 * q[1] + rz * q[2];
            q = this.G(b10 + bz); v = rx1 * q[0] + ry0 * q[1] + rz * q[2];
            a = this.lerp(sx, u, v);
            q = this.G(b01 + bz); u = rx0 * q[0] + ry1 * q[1] + rz * q[2];
            q = this.G(b11 + bz); v = rx1 * q[0] + ry1 * q[1] + rz * q[2];
            b = this.lerp(sx, u, v);
            c = this.lerp(sy, a, b);
            bz++;
            rz--;
    ...
    private G(i: number) { return this.points[i % 32]; }
    ...
    private init() {
        ...
        for (i = 0; i < this.B + 2; i++) {
            ...
            for (j = 0; j < 2; j++) { this.g2[this.B + i][j] = this.g2[i][j]; }
        }
